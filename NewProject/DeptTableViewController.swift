...
class DeptTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let imgback = UIImage(named:"background")
        let imgbackV = UIImageView(image:imgback)
        tableView.tableFooterView = UIView(frame: CGRect.zero)//empty cell
        //imgbackV.contentMode = .scaleAspectFit //set the original size of background
        tableView.backgroundColor = .lightGray

        self.tableView.backgroundView=imgbackV
        deptArray.append(deptInfo(name:"Computer Science",dept_id:"COMP"));
        deptArray.append(deptInfo(name:"Communications Studies",dept_id:"COMS"));
